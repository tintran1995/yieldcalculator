...
async function main() {
    ...
    const prices = await lookUpPrices(["pylon-finance", "ethereum", "solarite"]);
    const stakingTokenPrice = prices["pylon-finance"].usd;

    // const rewardTokenPrice = (await YFFI_DAI_BALANCER_POOL.getSpotPrice(LINK_TOKEN_ADDR, rewardTokenAddr) / 1e18) * stakingTokenPrice;
    const rewardTokenPrice = prices["solarite"].usd;
    // const rewardTokenPrice = 528;
    ...
    //await _print24HourPrice("shrimp", rewardTokenTicker);
